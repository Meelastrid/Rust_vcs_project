...
use std::fs::{self, metadata, write, File};
...
pub fn get_head_in_branch(branch: String) -> String {
    fs::read_to_string(PathBuf::from(".vcs").join("branches").join(branch)).unwrap()
}
...
pub fn get_workdir() -> String {
    fs::read_to_string(std::path::Path::new(".vcs").join("config")).unwrap()
}
...
pub fn get_parent(commit: String) -> std::result::Result<std::string::String, std::io::Error> {
    let vcs_dir = ".vcs/objects".to_string();
    let ref_prefix: String = commit[0..2].to_string();
    let ref_file_name: String = commit[2..].to_string();
    let contents = fs::read_to_string(PathBuf::from(vcs_dir).join(ref_prefix).join(ref_file_name))?;

    let mut lns = contents.lines();
    lns.next();
    Ok(lns.next().unwrap().to_string())
}
...
pub fn find_common_commit(branch1: String, branch2: String) -> String {
    ...
    while branch2_commit != "null" {
        ...
        while temp != "null" {
            ...
            temp = get_parent(temp).unwrap();
        }
        branch2_commit = get_parent(branch2_commit).unwrap();
    }
    "null".to_string()
}
...
pub fn merge(branch: String) -> std::io::Result<()> {
    ...
    let our_files_list: Vec<String> = get_files_in_branch("master".to_string())
        .keys()
        .cloned()
        .collect();
    let their_files: HashMap<String, String> = get_files_in_branch(branch.clone());
    let their_files_list: Vec<String> = get_files_in_branch(branch.clone())
        .keys()
        .cloned()
        .collect();
    let common_commit = find_common_commit("master".to_string(), branch.clone());

    let my_modified = check_modified_files_between_commits(
        get_head_in_branch("master".to_string()),
        common_commit,
    );
    let their_modified = check_modified_files_between_commits(
        get_head_in_branch("master".to_string()),
        get_head_in_branch(branch.clone()),
    );
    ...
    for fa in files_to_add.clone() {
        let mut f = File::open(
            ".vcs/objects/".to_string() + &their_files[&fa][0..2] + "/" + &their_files[&fa][2..],
        )
        .unwrap();
    ...
    for fm in their_modified.clone() {
        let mut f = File::open(
            ".vcs/objects/".to_string() + &their_files[&fm][0..2] + "/" + &their_files[&fm][2..],
        )
        .unwrap();
        let mut data = Vec::new();
        f.read_to_end(&mut data).unwrap();
