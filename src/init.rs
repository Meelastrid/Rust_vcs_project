// use util.rs;
use crate::{create_objects, create_commit, get_hash};
use std::env;
use std::fs;
...
pub fn init(args: &Vec<String>) -> std::io::Result<()> {
    ...
    create_commit(args[3].clone(), "master".to_string(), parent, "Initial commit".to_string());
    println!("Initialized VCS repository in {}", args[3]);
    println!("Created commit:\n[master {}] Initial commit", p_hash);
    Ok(())
}
